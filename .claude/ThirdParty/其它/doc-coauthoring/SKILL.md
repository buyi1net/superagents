...
description: 引导用户完成结构化文档协作：收集上下文、迭代组织内容，并用读者测试发现盲点。用户说"写个文档""写技术规范""整理 RFC""一起改文档""帮我润色结构"时使用；勿触发于把当前上下文生成并发布 PRD 的场景，那些走 to-prd。
---

# 文档协作工作流程

此技能提供一个结构化流程，带领用户完成协作文档创建。作为引导者，全程引导用户走完三个阶段：上下文收集、优化与结构化、读者测试。产出文档前，先加载 `zh-cn-mode` 以确保输出语言为简体中文。
...
**目标：** 缩小用户所知和 OpenCode 所知之间的认知差距，使后续能够提供智能指导。
...
告诉他们可以用简写回答，或者以任何最适合他们的方式尽量完整地提供上下文。

**如果用户提供模板或提到文档类型：**

- 询问他们是否有模板文档可以分享
- 如果他们提供共享文档的链接：若本环境已有可用工具则读取；否则要求用户粘贴内容或提供本地文件
- 如果他们提供本地文件，读取它

**如果用户提到编辑现有的共享文档：**

- 若本环境已有可用工具则读取当前状态；否则要求用户粘贴内容、提供本地文件，或手动导出为文本/markdown
- 检查没有 alt 文本的图像
- 如果存在没有 alt 文本的图像，解释当其他人使用 OpenCode 理解文档时，OpenCode 将无法看到它们。询问他们是否想生成 alt 文本。如果是，请求他们将每个图像粘贴到聊天中以生成描述性 alt 文本。

### 信息提供

一旦初始问题得到回答，鼓励用户尽量完整地提供所有上下文。请求信息如：

- 项目/问题的背景
- 相关的团队讨论或共享文档
- 为什么不使用替代方案
- 组织上下文（团队动态、历史背景、协作敏感点）
- 时间压力或限制
- 技术架构或依赖关系
- 利益相关者的担忧

建议他们不要担心整理它——先把信息直接发出来。提供多种方式来提供上下文：

- 意识流信息提供
- 指向要阅读的团队频道或帖子
- 链接到共享文档

**如果集成可用**可以直接使用这些来拉入上下文。若无可用工具，则要求用户粘贴内容或提供本地文件。

告诉他们一旦完成初始倾倒，将会提出澄清问题。

**在上下文收集期间：**

- 如果用户提到团队频道或共享文档：
  - 若本环境已有可用工具：告诉他们现在将读取内容
  - 若无可用工具：解释暂无法访问。要求用户直接粘贴相关内容、提供本地文件，或手动导出为文本/markdown。
...
当用户表明已完成信息提供（或在提供大量上下文后），提出澄清问题以确保理解：

根据上下文中的差距生成 5-10 个编号问题。

告诉他们可以用简写回答（例如，"1: 是，2: 否，3: 其他说明"），链接到更多文档，指向要阅读的频道，或者继续提供信息。任何对他们最有效的方式。
...
创建包含所有部分标题和占位符文本（如"[待编写]"或"[内容在此]"）的文件。

确认文件名已创建并表明是时候填写每个部分了。

**如本环境支持 artifacts，也可替代使用：**
创建 artifact，这给 OpenCode 和用户一个可以工作的脚手架。提供脚手架链接并表明是时候填写每个部分了。
...
告诉他们 [部分名称] 部分已在 [文件名] 中起草。让他们通读并指出要更改什么。
...
**如果可以访问子 agent（例如，在 OpenCode 中）：**

直接执行测试，无需用户参与。

### 步骤 1：预测读者问题

宣布打算预测读者在阅读或使用此文档时可能会问什么问题。
...
询问人们在阅读或使用此文档时可能会问什么问题。他们会向 OpenCode 输入什么？
...
- 起草完整部分
- 进行所有编辑
- 优先使用工作区 markdown 文件；如本环境支持 artifacts，也可使用 artifact 作为补充
