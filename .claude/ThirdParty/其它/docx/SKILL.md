---
name: docx
description: "创建、读取、编辑或分析 Word 文档（.docx），包括提取内容、查找替换、处理修订/评论、插入图片、生成带目录/页码/信头的正式文档；不要用于 PDF、电子表格、Google Docs 或通用编码任务。用户说\"生成 Word\"、\"处理 .docx\"、\"改一下这个 Word 文档\"、\"从 docx 提取内容\"、\"做个报告/备忘录/信件模板\"时使用。"
...
| 编辑现有文档 | 解包 → 编辑 XML → 重新打包 - 参考下方编辑现有文档部分 |
...
```bash
# Extract text including tracked changes
...
使用 JavaScript 生成 `.docx` 文件，然后验证。安装依赖：`npm install -g docx`

> **国内网络提示**：若 `npm install -g docx` 失败，请按团队规范配置 npm registry 或代理后再执行，不要未确认即修改全局配置。
...
**横向布局：** docx-js 会内部交换宽度/高度，因此传入纵向尺寸让它自动处理交换：

```javascript
size: {
  width: 12240,   // Pass short side as width
  height: 15840,  // Pass long side as height
  orientation: PageOrientation.LANDSCAPE  // docx-js swaps them in the XML
},
// Content width = 15840 - left margin - right margin (using the long side)
```
...
// ⚠️ Each reference creates an independent numbering sequence
// Same reference = continuous numbering (1,2,3 then 4,5,6)
...
new Table({
  ...
  columnWidths: [4680, 4680], // Sum must equal table width (DXA: 1440 = 1 inch)
  rows: [
    new TableRow({
      children: [
        new TableCell({
          borders,
          width: { size: 4680, type: WidthType.DXA }, // Each cell must also be set
          shading: { fill: "D5E8F0", type: ShadingType.CLEAR }, // CLEAR, not SOLID
          margins: { top: 80, bottom: 80, left: 120, right: 120 }, // Cell margins (internal, don't add to width)
...
columnWidths: [7000, 2360]  // Sum must equal table width
```
...
new Paragraph({
  children: [new ImageRun({
    ...
    altText: { title: "标题", description: "描述", name: "名称" } // All three properties required
  })]
})
```
...
```javascript
// IMPORTANT: PageBreak must be inside a Paragraph
...
// 1. Create a bookmark at the target
...
```javascript
// Right-aligned tab on same line (e.g. date on right side of a title)
new Paragraph({
  children: [
    new TextRun("公司名称"),
    new TextRun("\t2025年1月"),
...
// Dot leader (e.g. table of contents style)
new Paragraph({
  children: [
    new TextRun("简介"),
...
sections: [{
  properties: {
    column: {
      ...
      space: 720,        // Gap between columns in DXA (720 = 0.5 inches)
      equalWidth: true,
      separate: true,    // Show vertical line between columns
    },
  },
...
python scripts/comment.py unpacked/ 1 "回复文本" --parent 0  # Reply to comment 0
python scripts/comment.py unpacked/ 0 "文本" --author "自定义作者"  # Custom author name
```
...
<!-- Comment 0 contains nested reply 1 -->
<w:commentRangeStart w:id="0"/>
...
> **安装提示**：上述工具若安装或下载失败，请按团队规范配置镜像源或代理，不要未确认即修改全局配置。
