...
用法：
    python comment.py unpacked/ 0 "评论文本"
    python comment.py unpacked/ 1 "回复文本" --parent 0

文本应为预转义的 XML（例如 &amp; 代替 &，&#x2019; 代替智能引号）。

运行后，在 document.xml 中添加标记：
...
将标记嵌套在父评论 {pid} 的标记内（标记必须是 w:p 的直接子元素，绝不能放在 w:r 内部）：
...
def add_comment(
    ...
    action = "回复" if parent_id is not None else "评论"
    return para_id, f"已添加{action} {comment_id} (para_id={para_id})"


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="向 DOCX 文档添加评论")
    p.add_argument("unpacked_dir", help="已解包的 DOCX 目录")
    p.add_argument("comment_id", type=int, help="评论 ID（必须唯一）")
    p.add_argument("text", help="评论文本")
    p.add_argument("--author", default="Opencode", help="作者名称")
    p.add_argument("--initials", default="C", help="作者缩写")
    p.add_argument("--parent", type=int, help="父评论 ID（用于回复）")
    args = p.parse_args()
