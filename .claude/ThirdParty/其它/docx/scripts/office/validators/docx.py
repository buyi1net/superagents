...
class DOCXSchemaValidator(BaseSchemaValidator):
    ...
    def validate_deletions(self):
        ...
        if errors:
    print(f"失败 - 发现 {len(errors)} 个删除校验违规：")
    for error in errors:
        print(error)
    return False
else:
    ...
    def validate_insertions(self):
        ...
        if errors:
    print(f"失败 - 发现 {len(errors)} 个插入校验违规：")
    for error in errors:
        print(error)
    return False
else:
    ...
    print("通过 - 所有 paraId/durableId 值均符合约束")
    return True
        ...
        if errors:
    print(f"失败 - {len(errors)} 个评论标记违规：")
    for error in errors:
        print(error)
    return False
else:
    print("通过 - 所有评论标记已正确配对")
    return True
