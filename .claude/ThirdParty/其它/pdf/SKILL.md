---
name: pdf
description: 处理 PDF 文件：读取、提取文本/表格、合并、拆分、旋转、加水印、创建、填表、加密/解密、提取图片，以及对扫描版 PDF 做 OCR。用户说"处理 PDF""提取这个 PDF""合并几个 PDF""填 PDF 表单""生成 .pdf""把扫描件变成可搜索"时使用。
---
...
本指南涵盖使用 Python 库和命令行工具进行 PDF 处理的基本操作。如需高级功能、JavaScript 库和详细示例，请参阅 reference.md。如果需要填写 PDF 表单，请阅读 forms.md 并按照说明操作。
...
对于 canvas 绘制的文本（不是 Paragraph 对象），手动调整字体大小和位置，而不是使用 Unicode 下标/上标。
