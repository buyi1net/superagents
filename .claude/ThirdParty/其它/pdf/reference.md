...
本文档包含主 skill 说明中未涉及的高级 PDF 处理功能、详细示例和其他库。

> **示例说明**：代码示例中的 API 名称、变量名、文件名保留英文；示例文档正文（如 "Report Title"、"Quarterly Sales Report"）可按实际项目语言填写，此处保留以展示用法。输出文本和用户可见字符串可按团队语言调整。

## pypdfium2 库（Apache/BSD 许可证）

### 概述
pypdfium2 是 PDFium（Chromium 的 PDF 库）的 Python 绑定。它擅长快速 PDF 渲染、图片生成，可作为 PyMuPDF 的替代品。
...
### 2. 文本提取
- `pdftotext -bbox-layout` 是纯文本提取的最快方式
