---
name: playwright-cli
description: 使用 playwright-cli 自动化浏览器交互、检查网页状态，并辅助生成或调试 Playwright 测试。用户说"打开网页测一下"、"用浏览器点一下"、"跑 Playwright"、"生成端到端测试"、"检查页面交互"时使用。
...
每次命令执行后，playwright-cli 都会返回当前浏览器状态的快照。
...
默认使用快照中的元素引用（比如 e15 这种形式）来和页面元素交互。
...
你也可以使用 CSS 选择器或者 Playwright 内置选择器：
...
playwright-cli -s=mysession delete-data  # 删除持久化会话的用户数据（需确认后再执行）
...
如果本地版本可用，所有命令都可以用`npx playwright-cli`代替。

> ⚠️ 仅在用户明确确认后才执行全局安装：
> ```bash
> npm install -g @playwright/cli@latest
> ```
> 若安装失败，请按团队规范配置 npm registry 或代理，不要未确认即修改全局配置。
...
* **运行和调试 Playwright 测试用例** [references/playwright-tests.md](references/playwright-tests.md)
