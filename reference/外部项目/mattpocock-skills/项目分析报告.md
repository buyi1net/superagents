...
> 分析日期：2026-08-06  
> 快照状态：分析时使用过一次本地临时快照，当前仓库不保留该快照。
...
上游的 `package.json` 与 `.claude-plugin/plugin.json` 版本都是 `1.2.2`。`npm run check-plugin-version` 已实际执行并通过。
...
本报告基于 2026-08-06 对上游仓库一次本地临时快照的检查。快照当时位于 `D:/superagents` 的子目录中，不是独立 Git clone，因此不能从那份快照查看上游提交历史。当前仓库只保留分析结论，不保留快照内容。

复核报告中的版本、目录或文件时，应直接访问上游仓库，或另建真正的 clone；报告中出现的上游路径是资料描述，不是当前 `superagents` 的导航链接。

## 二、项目定位

上游的 `README.md` 把项目定位为一组可组合的工程 skill，用来处理 agent 开发中的几类常见问题：
...
项目没有运行时依赖。上游的 `package.json` 中只有 `@changesets/cli` 和 `@changesets/changelog-github` 两个开发依赖。
...
上游的 `CLAUDE.md` 明确规定，只有 `engineering/` 和 `productivity/` 可以进入 README、docs 和 plugin 清单。实测这 25 个正式 skill 与 `.claude-plugin/plugin.json` 完全一致，没有漏项或多发。
...
上游的 `skills/engineering/ask-matt/SKILL.md` 是整套系统的 router。它不直接完成工程任务，而是告诉用户当前应该走哪条路径。
...
上游的 `skills/productivity/writing-for-agents/SKILL.md` 提出了一套适合继续研究的写作方法：
...
上游的 `.github/workflows/release.yml` 只执行 `npm ci` 和 Changesets 发版，没有自动执行：
...
当时状态是自洽的，但主要依赖维护者手工遵守上游的 `CLAUDE.md`。这类仓库最容易出现的不是运行时 bug，而是规则漂移和清单漏项。
...
其中上游的 `scripts/link-skills.sh` 虽然标明仅供维护者使用，但删除目标前没有备份或确认，不能直接用于 `superagents`。
...
上游的 `README.md` 和 `tdd` 的 description 仍使用 `red-green-refactor`，但上游的 `skills/engineering/tdd/SKILL.md` 明确说 refactor 不属于循环。
...
上游的 `.agents/adr/0002-ship-as-a-claude-code-plugin.md` 说明了没有原生 Codex plugin 的原因：
...
1. 上游的 `README.md`：了解作者试图解决的问题。
2. 上游的 `CLAUDE.md`：了解仓库的发布和维护不变量。
3. 上游的 `skills/engineering/ask-matt/SKILL.md`：掌握全部 skill 的关系图。
4. 上游的 `skills/productivity/writing-for-agents/SKILL.md` 与 `SKILL-MECHANICS.md`：研究 skill 写作方法。
5. 上游的 `skills/engineering/diagnosing-bugs/SKILL.md`：研究反馈回路和调试阶段门禁。
6. 上游的 `skills/engineering/tdd/SKILL.md` 与 `code-review/SKILL.md`：研究实现后的验证方式。
7. 上游的 `skills/engineering/codebase-design/SKILL.md` 与 `domain-modeling/SKILL.md`：研究共享词汇和模块设计。
