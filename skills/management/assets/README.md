...
```
node gov.mjs init [项目名]        # 新项目初始化
node gov.mjs adopt                # 既有项目接入，生成 manifest 草稿
node gov.mjs add-reference <url> [--category 分类] [--name 名称] [--snapshot 快照] [--note 用途]
node gov.mjs sync                 # 从 manifest 重新生成索引段与参考说明
node gov.mjs check                # 一致性校验，红灯即欠账
```
...
| 位置 | 字段 | 说明 |
|---|---|---|
| project | name / summary | 项目名（必填）、一句话用途 |
