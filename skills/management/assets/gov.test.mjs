...
const run = (args, cwd) => spawnSync(process.execPath, ['.agents/gov/gov.mjs', ...args], { cwd, encoding: 'utf8' });
...
test('init 后 check 全绿', () => {
  ...
  assert.ok(fs.existsSync(path.join(WS, '.agents/gov/gov.mjs')), 'init 应把 gov.mjs 种进 .agents/gov/');
  assert.ok(fs.existsSync(path.join(WS, '.agents/gov/manifest.json')), 'manifest 应在 .agents/gov/ 下');
...
test('add-reference 全链路：克隆、登记、忽略、生成、check 绿', () => {
  ...
  const m = JSON.parse(r('.agents/gov/manifest.json'));
...
test('TODO 字段与磁盘未登记参考项都会亮红灯', () => {
  ...
  w('.agents/gov/manifest.json', JSON.stringify(m, null, 2));
